...
**/
public class BamSHA512ChecksumRecordHandler implements BamRecordHandler
{
	BamHeaderParser BHP;
	...
	public BamSHA512ChecksumRecordHandler(BamHeaderParser BHP) throws Exception
	{
		this.BHP = BHP;
	...
	public String toString()
	{
		...
		SB.append("all\tall\t" +
			allcnt + "\t\t" +
			(crc_all_flags_seq) + "\t" +
			(crc_all_name_flags_seq) + "\t" +
			(crc_all_flags_seq_qual) + "\t" +
			(crc_all_flags_seq_tags) + "\n"
		);
		SB.append("all\tpass\t" +
			passcnt + "\t\t" +
			(crc_pass_flags_seq) + "\t" +
			(crc_pass_name_flags_seq) + "\t" +
			(crc_pass_flags_seq_qual) + "\t" +
			(crc_pass_flags_seq_tags) + "\n"		
		);
		...
		if ( rg_crc_all_name_flags_seq.length > 1 )
			for ( int i = 0; i < rg_crc_all_name_flags_seq.length; ++i )
			{
				SB.append(
					...
					rg_allcnt[i] + "\t\t" +
					(rg_crc_all_flags_seq[i]) + "\t" +
					(rg_crc_all_name_flags_seq[i]) + "\t" +
					(rg_crc_all_flags_seq_qual[i]) + "\t" +
					(rg_crc_all_flags_seq_tags[i]) + "\n"			
				);		
				SB.append(
					BHP.getReadGroupId(i-1)+"\tpass\t" +
					rg_passcnt[i] + "\t\t" +
					(rg_crc_pass_flags_seq[i]) + "\t" +
					(rg_crc_pass_name_flags_seq[i]) + "\t" +
					(rg_crc_pass_flags_seq_qual[i]) + "\t" +
					(rg_crc_pass_flags_seq_tags[i]) + "\n"			
				);		
			}
